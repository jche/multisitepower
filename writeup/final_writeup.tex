...
\begin{figure}[ht]
	...
	\caption{Average margin of error ($J=10,25,100$, $\tau=0.2$, $\sigma_\tau=0.2$, $ICC=0.2$).}
	\label{fig:results_J}
\end{figure}
...
\begin{figure}[ht]
	...
	\caption{Average margin of error ($J=25$, $\tau=0.2$, $\sigma_\tau=0.1, 0.2, 0.3$, $ICC=0.2$).}
...
Overall, simulation factors..

Sample size in site is the most important.

% TODO: highlight importance of correct interpretation?
...
[TODO: model this based on some application of interest, wait for Mike's example]


\section{Conclusion}

In many settings, analysts may wish to power multisite trials to consistently detect effects at individual sites.
While this task appears straightforward at first glance, it is important to carefully consider it interacts with the shrinkage models we use to estimate site-level treatment effects.
